...
* `encodingTool`: (MP4 atom: `©too`)
* `executiveProducer`: (MP4 atom: `©xpd`)
* `filmMakerWebpage`: (MP4 atom: `©mal`)

**G-L**
* `gaplessPlayback`: (MP4 atom: `pgap`)
* `genre`: (MP4 atom: `©gen` - ID3 frame: `TCON`)
...
* `publisherWebpage`: (MP4 atom: `©lal` - ID3 frame: `WPUB`)
...
* `recordingDate`: (MP4 atom: `©day` - ID3 frame: `TDRC`)
* `recordingCopyright aka Phonogram Rights`: (MP4 atom: `©phg`) 
