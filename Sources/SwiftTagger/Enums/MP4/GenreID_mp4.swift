...
public enum Genres_Mp4: Int, CaseIterable {
    case unknown = 0
    case musicBlues = 2
    case musicComedy = 3
    ...
    var stik: Stik? {
        if Genres_Mp4.musicSet.contains(self.rawValue) {
            return .music
        } else if Genres_Mp4.podcast.contains(self.rawValue) {
            return .podcast
        } else if Genres_Mp4.musicVideosSet.contains(self.rawValue) {
            return .musicVideo
        } else if Genres_Mp4.tvShow.contains(self.rawValue) {
            return .tvShow
        } else if Genres_Mp4.movie.contains(self.rawValue) {
            return .movie
        } else if Genres_Mp4.ringtone.contains(self.rawValue) {
            return .ringtone
        } else if Genres_Mp4.audiobook.contains(self.rawValue) {
            return .audiobook
        } else if Genres_Mp4.iTunesUSet.contains(self.rawValue) {
            return .iTunesU
        } else {
            return nil
        }
    ...
    static let nameMapping: [String: Genres_Mp4] = {
        var mapping: [String: Genres_Mp4] = [:]
        for genre in Genres_Mp4.allCases {
    ...
    init?(genreName: String) {
        self = Genres_Mp4.nameMapping[genreName] ?? .unknown
    }
