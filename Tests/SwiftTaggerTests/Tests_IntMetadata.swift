...
final class SwiftTaggerTests_IntMetadata: XCTestCase {
    ...
    func testReadWriteMp4IntMetadata() throws {
        ...
        XCTAssertEqual(read.albumID, 12345678)
        XCTAssertEqual(read.appleStoreCountryID, 23456789)
        XCTAssertEqual(read.artistID, 34567890)
        XCTAssertEqual(read.bpm, 99)
        XCTAssertEqual(read.composerID, 45678901)
        XCTAssertEqual(read.conductorID, 56789012)
        XCTAssertEqual(read.genreID.mp4Genre, .audiobooks)
        XCTAssertNil(read.genreID.id3Genre)
        XCTAssertEqual(read.length, 5016)
        XCTAssertEqual(read.movementCount, 5)
        XCTAssertEqual(read.movementNumber, 4)
        XCTAssertEqual(read.playlistID, 67890123)
        XCTAssertEqual(read.tvEpisodeNumber, 9)
        XCTAssertEqual(read.tvSeason, 1)
...
//        let outputUrl = try localDirectory(fileName: "testMp4-Int", fileExtension: "m4a")
    ...
    func testReadWriteId3IntMetadata() throws {
        ...
        XCTAssertEqual(read.length, 5041)
